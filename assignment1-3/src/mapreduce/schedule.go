...
type TaskResult struct {
	status     int // -ve if task failed , +ve if succeed, absolute value indicated the task number
	workerName string
}
...
func assignToWorker(mr *Master, workers *Workers, started *bitset.BitSet,
	mutex *sync.Mutex, phase jobPhase, ntasks int, nios int, statusChan chan TaskResult) {
	/*
...
func startPhase(phase jobPhase, mr *Master, workers *Workers,
	ntasks int, nouts int, done chan bool) {
